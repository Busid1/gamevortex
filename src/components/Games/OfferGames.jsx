...
export default function OfferGames({ handleAddToCart, handleRemoveFromCart }) {
    const videogames = useVideogames();
    const cardGamesContentRef = useRef(null);

    const scrollLeft = () => {
        cardGamesContentRef.current.scrollBy({
            left: -100, // Ajusta la cantidad de desplazamiento según sea necesario
            behavior: 'smooth'
        });
    };
    ...
    const scrollRight = () => {
        cardGamesContentRef.current.scrollBy({
            left: 200, // Ajusta la cantidad de desplazamiento según sea necesario
            behavior: 'smooth'
        });
    };
    ...
    return (
        <div id="cardGames-box" className="px-3">
            <h2 className="text-white m-0">Offers</h2>
